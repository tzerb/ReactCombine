...
export default function tripReducer(state = initialState.trips, action : any) {
    ...
    switch(action.type) {
        ...
        if (action.waypoint.tripId && action.waypoint.tripId!=0)
        {
            ...
            newTrip.waypoints = [...newTrip.waypoints.filter((wp:any) => wp.waypointId !== action.waypoint.waypointId)];
            let newState = [
        ...
        if (action.waypoint.tripId && action.waypoint.tripId!=0)
        {
            let tripsWithWaypoint = state.filter(trip => trip.tripId == action.waypoint.tripId);
            let newTrip = Object.assign({}, tripsWithWaypoint[0]);
            newTrip.waypoints = [...newTrip.waypoints.filter((wp:any) => wp.waypointId !== action.waypoint.waypointId), Object.assign({}, action.waypoint)];
            let newState = [
                ...state.filter(trip => trip.tripId !== action.waypoint.tripId),
                newTrip
            ];
        ...
        if (action.picture.tripId && action.picture.tripId!=0)
        {
            ...
            newTrip.pictures = [...newTrip.pictures.filter((wp: any) => wp.pictureId !== action.picture.pictureId)];
            let newState = [
        ...
        if (action.picture.tripId && action.picture.tripId!=0)
        {
            ...
            newTrip.pictures = [...newTrip.pictures.filter((wp: any) => wp.pictureId !== action.picture.pictureId), Object.assign({}, action.picture)];
            let newState = [
                ...state.filter(trip => trip.tripId !== action.picture.tripId),
                newTrip
            ];
